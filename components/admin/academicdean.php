...
if (isset($_POST['submit'])) {
    ...
    $check_email_sql = "SELECT * FROM users WHERE email = '$email' AND role ='Academic_Dean' ";
    ...
    if ($result->num_rows > 0) {
        ...
        if ($conn->query($sql) === TRUE) {
            $message = "Academic Dean Added Successfully";
        } else {
...
if (isset($_POST['edit'])) {
    ...
    if ($result) {
        $message  = "Academic Dean Updated successfully.";
    } else {
...
if (isset($_POST['delete'])) {
    ...
    $result = mysqli_query($conn, $query);
    if ($result) {
    $message = "Academic Dean Deleted successfully.";
    } else {
    $message = "Error: " . mysqli_error($conn);
    }
    }
$dean_query = "SELECT * FROM users WHERE role = 'Academic_Dean' ";
...
<div class="breadcrumb-line breadcrumb-line-light header-elements-lg-inline mt-3">
  <div class="d-flex">
      <div class="breadcrumb">
          ...
          <span class="breadcrumb-item active">Academic Dean</span>
      </div>
      <a href="#" class="header-elements-toggle text-body d-lg-none"><i class="icon-more"></i></a>
  </div>
  <div class="d-flex">
      <div class="breadcrumb">
           <a class="breadcrumb-item" data-toggle="modal" data-target="#myModaldescription"><i class="icon-user mr-2"></i>Add Academic Dean</a>
      </div>
...
if (mysqli_num_rows($dean_result) > 0) {
  echo '
  <div class="content">
  <div class="card">
...
<h5>' . $message. '</h5>
  ...
  echo'<table class="table datatable-button-html5-columns">
          ...
          <tbody>';
  while ($row = mysqli_fetch_assoc($dean_result)) {
      echo '<tr>
              <th>' . $row['id'] . '</th>
                    ...
                    <div class="dropdown">
                        ...
                        <form method="POST" action="">
                            ...
                            <button type="submit" name = "delete" class="dropdown-item" onclick="return confirm("Are you sure you want to delete this user?")">
                                <i class="fas fa-trash-alt"></i> Delete Academic Dean
                            </button>
                        </form>
                        <a class="dropdown-item" data-toggle="modal" data-target="#myModaldescription_'.$row['id'].' ">
                            <i class="fas fa-edit"></i> Edit Academic Dean
                        </a>';
                        if ($row['status'] == 0) {
