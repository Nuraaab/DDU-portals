...
if (isset($_POST['submit'])) {
    ...
    $check_email_sql = "SELECT * FROM users WHERE email = '$email' AND role ='CDE_Officer' ";
...
$cde_query = "SELECT * FROM users WHERE role = 'CDE_Officer' ";
$cde_result = mysqli_query($conn, $cde_query);
...
<div class="breadcrumb-line breadcrumb-line-light header-elements-lg-inline mt-3">
  ...
  <div class="d-flex">
      <div class="breadcrumb">
           <a class="breadcrumb-item" data-toggle="modal" data-target="#myModaldescription"><i class="icon-user mr-2"></i>Add CDE Officer</a>
      </div>
...
if (mysqli_num_rows($cde_result) > 0) {
  echo '
  <div class="content">
  <div class="card">
...
<h5>' . $message. '</h5>
  ...
  echo'<table class="table datatable-button-html5-columns">
          ...
          <tbody>';
  while ($row = mysqli_fetch_assoc($cde_result)) {
      echo '<tr>
              <th>' . $row['id'] . '</th>
                    ...
                    <div class="dropdown">
                        ...
                        <form method="POST" action="">
                            ...
                            <button type="submit" name = "delete" class="dropdown-item" onclick="return confirm("Are you sure you want to delete this user?")">
                                <i class="fas fa-trash-alt"></i> Delete CDE Officer
                            </button>
                        </form>
                        <a class="dropdown-item" data-toggle="modal" data-target="#myModaldescription_'.$row['id'].' ">
                            <i class="fas fa-edit"></i> Edit CDE Officer
                        </a>';
                        if ($row['status'] == 0) {
